...
%%% Computes backgroundF during light stim (useless with new interline)
...
%Index for later is    ON-OFF = 2       ON = 3       BAD = 4
OnOffCells =  [2,20,22,34,36,38,51,59,63,69,78,85,89,95,101];
OnCells = [24,53,81,84,96];
badCells = [1,3:19,21,23,25:33,35,37,39:50,52,54,55,56,57,58,60,61,62,64:68,70:77,79,80,82,83,86,87,88,90,91,92,93,94,97:100,102,103,104];
...
for i=1:n_frames
    ...
    for j = 1:max(max(countmask))
        ...
        cellCenters(j,:) = partprops(j).Centroid/256*lengthOfFOV-0.5*lengthOfFOV;
    end
...
% pathName = 'C:\Users\Olive\Dropbox\DS project\3_masterResults\Data\';
...
%% Now plot DSI v VS   and     maxDF histogram to check threshold values
...
%% Now map out where the DS cells are in the retina
